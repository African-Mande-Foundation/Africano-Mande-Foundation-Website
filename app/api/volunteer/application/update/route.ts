...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('Updating application for email:', session.user.email);
    ...
    console.log('Using user ID for application update:', userId);
    ...
    // For updates, failed uploads are not critical (user might not be changing files)
    if (failedUploads.length > 0) {
      console.log(`Some files failed to upload: ${failedUploads.join(', ')}`);
    }
    ...
    console.log('Application data to update:', Object.keys(applicationData));
    ...
    if (!existingResponse.ok) {
      return NextResponse.json({ 
        success: false, 
        error: 'Could not find existing application to update.'
      });
    }

    const existingData = await existingResponse.json();
    const existingApplication = existingData.data?.[0];

    if (!existingApplication) {
      return NextResponse.json({ 
        success: false, 
        error: 'No existing application found to update.'
      });
    }

    // Update existing application using documentId (Strapi v5 uses documentId for updates)
    const updateId = existingApplication.documentId || existingApplication.id;
    console.log('Updating existing application with documentId:', updateId);
    
    const response = await fetch(
      `${process.env.STRAPI_URL}/api/volunteer-applications/${updateId}`,
      {
        method: 'PUT',
        headers: {
    ...
    if (failedUploads.length > 0) {
      message += ` (Note: ${failedUploads.join(', ')} file(s) were not updated)`;
    }
