...
export default function Sidebar({
  ...
  const navItems = [
    { icon: LayoutDashboard, label: "Dashboard", href: "/volunteer" },
    { icon: MessageSquare, label: "Remarks", href: "/volunteer/remarks" },
    { icon: Newspaper, label: "Updates", href: "/volunteer/updates" },
    { icon: LogOut, label: "Membership Portal", href: "/membership" },
  ];
  ...
  return (
    <>
      ...
      >
        ...
        />
        <div
          className={`w-2/3 absolute flex flex-col gap-y-5 z-40 h-full left-0 top-0 bg-[#3B6907] p-2 transform transition-all duration-500 ease-out ${
      ...
      {/* Desktop Sidebar */}
      <div
        className={`hidden md:block h-screen bg-[#3B6907] transition-all duration-700 ease-in-out ${
          isMobileMenuOpen ? "w-1/4 lg:w-1/5 xl:w-1/7" : "w-16"
        }`}
      >
        <div className="w-full flex flex-col gap-y-5 h-full bg-[#3B6907] p-2">
          <div
